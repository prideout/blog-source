...
try:
    shutil.rmtree('CurlNoise')
except:
    pass

print "Exporting SVN repo..."
os.system('svn export .. CurlNoise')

os.remove('CurlNoise/doc/Post.html')
os.remove('CurlNoise/doc/UpdateBlog.py')
for p in glob.glob('CurlNoise/doc/*.png'):
    os.remove(p)
...
zipper("CurlNoise", 'CurlNoise.zip')
...
if False:
    SendFile(ftp, "CurlNoise.zip")
    SendFile(ftp, "CurlNoise/assets/Shaders.glsl")
    SendFile(ftp, "*.png")
    SendFile(ftp, "curl.dot.svg")

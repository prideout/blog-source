...
postid = 44 ####################### BE SURE TO UPDATE THIS !!!!!!!!!!!
...
os.system('copy ..\\*.py .')
os.system('copy ..\\screenshots\*.png .')

print "Zipping..."

def zipper(pattern, zip_file):
    zip = zipfile.ZipFile(zip_file, 'w', compression=zipfile.ZIP_DEFLATED)
    files = glob.glob(pattern)
    files.remove("UpdateBlog.py")
    for f in files:
        zip.write(f, f, zipfile.ZIP_DEFLATED)
    zip.close()
    return zip_file

def tarballer(pattern, tar_file):
    ...
    files.remove("UpdateBlog.py")
    for f in files:
        t.add(f, f, zipfile.ZIP_DEFLATED)
    t.close()
    return t

zipper("*.py", 'meshgen.zip')
tarballer("*.py", 'meshgen.tar.gz')
...
SendFile(ftp, "meshgen.zip")
SendFile(ftp, "meshgen.tar.gz")
SendFile(ftp, "*.png")
SendFile(ftp, "*.py")

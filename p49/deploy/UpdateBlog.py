...
try:
    shutil.rmtree('QuadTess')
except:
    pass

print "Exporting SVN repo..."
os.system('svn export .. QuadTess')

if True:
    try:
        shutil.rmtree('QuadTess/deploy')
    except:
        pass
...
def zipper(dir, zip_file):
    ...
    for root, dirs, files in os.walk(dir):
        ...
        for f in files:
            if not f.endswith('.png') and not f.endswith('.gif'):
...
zipper("QuadTess", 'QuadTess.zip')
tarballer("QuadTess", 'QuadTess.tar.gz')
...
SendFile(ftp, "QuadTess/BicubicPatch.*")
SendFile(ftp, "QuadTess/gumbo.h")
SendFile(ftp, "QuadTess.zip")
SendFile(ftp, "QuadTess.tar.gz")
SendFile(ftp, "QuadTess/*.png")

...
postid = 36 ####################### BE SURE TO UPDATE THIS !!!!!!!!!!!
...
os.system('svn export .. pez')

if True:
    try:
        shutil.rmtree('pez/deploy')
    except:
        pass
...
def tarballer(dir, tar_file):
    t = tarfile.TarFile(tar_file, 'w')
    t.close()

    t = tarfile.TarFile.open(tar_file, 'w:gz')
    root_len = len(os.path.abspath(dir))
    for root, dirs, files in os.walk(dir):
        archive_root = os.path.abspath(root)[root_len:]
        for f in files:
            fullpath = os.path.join(root, f)
            archive_name = os.path.join(archive_root, f)
            t.add(fullpath, archive_name, zipfile.ZIP_DEFLATED)
    t.close()
    return t

zipper("pez", 'pez.zip')
tarballer("pez", 'pez.tar.gz')
...
SendFile(ftp, "PezCostume.jpg")
SendFile(ftp, "PezTriangle.png")
SendFile(ftp, "PezTexture.png")
SendFile(ftp, "PezMesh.png")
SendFile(ftp, "pez.zip")
SendFile(ftp, "pez.tar.gz")
SendFile(ftp, "pez/lib/libpez/pez.h")
SendFile(ftp, "pez/lib/libpez/pez.cocoa.m")
SendFile(ftp, "pez/lib/libpez/pez.linux.c")
SendFile(ftp, "pez/lib/libpez/pez.windows.c")

demoFiles = ['Gandhi.png', 'PezIntro1.Clear.c', 'PezIntro3.Texture.c',
	     'PezTexture.png', 'Simple.glsl', 'HeadlessGiant.ctm',
	     'PezIntro2.Triangle.c', 'PezIntro4.Mesh.cpp', 'PixelLighting.glsl']

for file in demoFiles:
    SendFile(ftp, "../demo/" + file)

...
struct RenderContext
{
    ...
    Vertex TransformedVerts[VertexCount];
};
...
static void UpdateVertexBuffer()
{
    ...
    {
        for (int useless = 0; useless < 8; ++useless)
        {
            rc.TransformedVerts[i].Normal = rc.NormalMatrix * rc.Verts[i].Normal;
            rc.TransformedVerts[i].Position = rc.Projection * (rc.Modelview * rc.Verts[i].Position);
        }
    }
    ...
    if (first)
    {
        ...
        const GLvoid* data = rc.TransformedVerts[0].Position.Pointer();
        glBindBuffer(GL_ARRAY_BUFFER, rc.VertexBuffer);
